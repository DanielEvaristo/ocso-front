...
export default async function ProductPage({
  ...
  const responseProduct = await fetch(`${API_URL}/products/${params.id}`, {
    method: "GET",
    headers: { ...authHeaders() },
    next: { tags: [`dashboard:products:${params.id}`] },
  });
  ...
  return (
    <div className="w-full">
      ...
      <UpdateProduct product={product} providers={providers}></UpdateProduct>
      <div className="pl-10">
        <DeleteProduct productId={product.productId}></DeleteProduct>
      </div>
    </div>
  );
}